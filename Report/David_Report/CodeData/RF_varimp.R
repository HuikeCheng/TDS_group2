...
### load data
RF.myeloid.model <- readRDS("RF.myeloid.model.rds")
RF.all.model <- readRDS("RF.all.model.rds")
RF.lymph.model <- readRDS("RF.lymph.model.rds")
RF.other.model <- readRDS("RF.other.model.rds")
...
df_mye <- importance(RF.myeloid.model) %>% 
  data.frame() %>% 
...
df_mye$varname <- rownames(df_mye)
...
top5 <- order(df_mye$MeanDecreaseGini, decreasing = TRUE)[1:5]
...
################## ALL ########################
df_all <- importance(RF.all.model) %>% 
  data.frame() %>% 
  mutate(feature = row.names(.))
...
df_all$varname <- rownames(df_all, 5)
...
top5 <- order(df_all$MeanDecreaseGini, decreasing = TRUE)[1:5]
...
######################## Lymphoid #######################################
df_lym <- importance(RF.lymph.model) %>% 
  data.frame() %>% 
  mutate(feature = row.names(.))
...
df_lym$varname <- rownames(df_lym, 5)
...
top5 <- order(df_lym$MeanDecreaseGini, decreasing = TRUE)[1:5]
...
################################## Other #######################################
df_oth <- importance(RF.other.model) %>% 
  data.frame() %>% 
  mutate(feature = row.names(.))
...
df_oth$varname <- rownames(df_oth, 5)
...
df_oth <- df_oth[,c(4,6)]
df_oth$model <- rep("Other", 28)
top5 <- order(df_oth$MeanDecreaseGini, decreasing = TRUE)[1:5]
df_oth$criteria <- rep(0, 28)
...
plotVarImp3(data = df, varname = 2, varimp = 1, model = 3, criteria = 4)
#saveRDS(df, "../../../output/RF_varimp.rds")
