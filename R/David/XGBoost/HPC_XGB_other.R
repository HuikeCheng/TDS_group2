...
#tune_grid <- expand.grid(nrounds = 200,                                #turn on for toy set
  #           max_depth = 4,
   #          eta = 0.05,
    #         gamma = 0.01,
...
#xgb_model <- train(x = train_x,                                        #turn on for toy set
 #           y = train_y, 
  #          method = "xgbTree",
...
saveRDS(train_pred_probs, "David/model_output/xgb.other.train_prob.rds")

train_pred <- predict(xgb_model, newdata = train_x)
train_pred <- as.data.frame(train_pred)
saveRDS(train_pred, "David/model_output/xgb.other.train_pred.rds")

train_y <- as.data.frame(train_y)
saveRDS(train_y, "David/model_output/xgb.other.train_y.rds")
...
a <- ggplot(plt, aes(Reference,Prediction, fill= Freq)) +
  ...
  labs(x = "Reference",y = "Prediction", title = "Confusion Matrix for XGBoost for Group Other") +
  scale_x_discrete(labels=c("Negative","Positive")) +
  scale_y_discrete(labels=c("Positive","Negative"))
a
ggsave(filename="../output/XGB_other_cm.plot.pdf", plot=a)
...
varimp_plot <- plot(varimp, main="Variable Importance with XGBoost Group Other")
varimp_plot

pdf("../output/XGB_other_varimp_plot.pdf")
print(varimp_plot)
dev.off()
...
b <- ggroc(roc_obj, colour = 'steelblue', size = 2) +
  ggtitle(paste0('ROC Curve for XGBoost for Group Other ', '(AUC = ', auc, ')'))
b
ggsave(filename="../output/XGB_other_roc.plot.pdf", plot=b)
...
xgb.other.varimp <- varImp(xgb_model)$importance
saveRDS(xgb.other.varimp, "David/model_output/xgb.other.varimp.rds")
saveRDS(xgb_model, "David/model_output/xgb.other.xgb_model.rds")

test_y <- as.data.frame(test_y)
saveRDS(test_y, "David/model_output/xgb.other.test_y.rds")

xgb.pred <- as.data.frame(xgb.pred)
saveRDS(xgb.pred, "David/model_output/xgb.other.pred.rds")

pred_probs <- as.data.frame(pred_probs)
saveRDS(pred_probs, "David/model_output/xgb.other.pred_probs.rds")
