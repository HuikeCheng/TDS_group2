...
saveRDS(train_pred, "David/model_output/RF.lymph.train_pred.rds")

train_pred_probs <- predict(custom, train,  type='prob')
train_pred_probs <- as.data.frame(train_pred_probs)
saveRDS(train_pred_probs, "David/model_output/RF.lymph.train_pred_probs.rds")

train_y <- as.data.frame(train_y)
saveRDS(train_y, "David/model_output/RF.lymph.train_y.rds")

train$lymphoid <- as.data.frame(train$lymphoid)
saveRDS(train$lymphoid, "David/model_output/RF.lymph.train.lymphoid.rds")
...
a
ggsave(filename="../output/RF_Lymph_cm.plot.pdf", plot=a)
...
pdf("../output/RF_Lymph_varimp_plot.pdf")
print(varimp_plot)
dev.off()
...
b <- ggroc(roc_obj, colour = 'steelblue', size = 2) +
  ggtitle(paste0('ROC Curve with Random Forest for Group Lymphoid', '(AUC = ', auc, ')'))
b
ggsave(filename="../output/RF_Lymph_roc.plot.pdf", plot=b)


#Save output for visualisation locally

RF.lymph.varimp <- varImp
saveRDS(RF.lymph.varimp, "David/model_output/RF.lymph.varimp.rds")

saveRDS(custom, "David/model_output/RF.lymph.model.rds")

test$lymphoid <- as.data.frame(test$lymphoid)
saveRDS(test$lymphoid, "David/model_output/RF.lymph.test$lymphoid.rds")

test_y <- as.data.frame(test_y)
saveRDS(test_y, "David/model_output/RF.lymph.test_y.rds")

pred <- as.data.frame(pred)
saveRDS(pred, "David/model_output/RF.lymph.pred.rds")

pred_probs <- as.data.frame(pred_probs)
saveRDS(pred_probs, "David/model_output/RF.lymph.pred_probs.rds")
